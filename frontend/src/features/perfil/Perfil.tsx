...
export default function Perfil() {
  ...
  async function onChangePassword(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      // Intentar varios endpoints y detectar contraseña actual incorrecta (400/401)
      const payload = { currentPassword: pwd.current, newPassword: pwd.next };
      const attempts = [
        () => api.post('/auth/change-password', payload),
        () => api.put(`/api/users/${user?.id}/password`, payload),
        () => api.patch('/api/users/me/password', payload),
      ];
      ...
      for (const run of attempts) {
        try {
          ...
          if (status === 400 || status === 401 || msg.includes('actual') || msg.includes('current') || msg.includes('incorrect') || msg.includes('inválid') || msg.includes('errónea')) {
            setPwdMsg('Contraseña actual errónea');
            setPwdLocked(true);
            return; // no seguir intentando otros endpoints
          }
          // Otros errores: seguir intentando siguiente endpoint
        }
      }
      // Si llegó aquí, todos fallaron por otra razón
      throw lastError;
    } catch (e: any) {
      setPwdMsg(e?.response?.data?.message || 'No se pudo cambiar la contraseña');
    } finally {
      setSaving(false);
    }
  }
